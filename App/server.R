...
server <- function(input, output, session) {
  ...
  stock_info <- reactive({
    ...
    get_Stock_info(filtered_row$SpeciesCommonName[1], filtered_row$SpeciesScientificName[1], query$stockkeylabel, SAG_data_reactive()$AssessmentYear[1], query$assessmentcomponent, SAG_data_reactive()$StockDescription[1], filtered_row$EcoRegion, query$assessmentkey)
  })
